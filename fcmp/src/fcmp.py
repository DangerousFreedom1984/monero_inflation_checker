...
from circuit import (
    Circuit, DlogParams, CurveSpec, GeneratorTable,
    OC_PARAMS, C1_PARAMS, C2_PARAMS,
    PointWithDlog, Divisor,
)
...
class ScalarMulAndDivisor:
    ...
    For C1 curve: generator is WPoint(Selene), coordinates are HelioseleneField.
    ...
    def new_c1(cls, A: WPoint, scalar_decomp: ScalarDecomposition):
        ...
        return cls(p, HelioseleneField(p.x.v), HelioseleneField(p.y.v), divisor)
    ...
    def new_c2(cls, A: WPoint, scalar_decomp: ScalarDecomposition):
        ...
        A           : WPoint on Helios
        scalar_decomp: ScalarDecomposition (must be over the C2 scalar field = HelioseleneField)
        """
        p = A * scalar_decomp.scalar
...
class IBlind:
    ...
    """
    def __init__(self, scalar: ScalarDecomposition,
                 u: ScalarMulAndDivisor, v: ScalarMulAndDivisor):
        self.scalar = scalar
        self.u = u
        self.v = v
...
class OutputBlinds:
    ...
    def __init__(self, o_blind: OBlind, i_blind: IBlind,
                 i_blind_blind: IBlindBlind, c_blind: CBlind):
...
class PerInputData:
    ...
    def __init__(self, output: Output, output_blinds: OutputBlinds,
                 branches_c1: list, branches_c2: list, leaves=None):
        self.output         = output
        self.output_blinds  = output_blinds
        self.branches_c1    = branches_c1   # list of lists of HelioseleneField (Selene field)
        self.branches_c2    = branches_c2   # list of lists of HeliosField (Helios field)
        self.leaves         = leaves        # list of HelioseleneField (6*LAYER_ONE_LEN elements)
...
class RootBranchC1:
    def __init__(self, branch: list):
        self.branch = branch   # list of HelioseleneField
...
class FcmpParams:
    ...
    C1 = Selene (base field HelioseleneField, scalar field HeliosField)
    C2 = Helios (base field HeliosField, scalar field HelioseleneField)
...
class Fcmp:
    ...
    @staticmethod
    def transcript(tree_root_is_c1: bool, tree_root_bytes: bytes,
                   inputs: list, root_blind_R: bytes) -> bytes:
        ...
        for inp in inputs:
            for f in [inp["O_tilde"][0], inp["O_tilde"][1],
                      inp["I_tilde"][0], inp["I_tilde"][1],
                      inp["R"][0],       inp["R"][1],
                      inp["C_tilde"][0], inp["C_tilde"][1]]:
                h.update(f.to_bytes())
        # Root blind nonce
    ...
    @staticmethod
    def _transcript_branches(branches: BranchesWithBlinds,
                             c1_tape: VectorCommitmentTape,
                             c2_tape: VectorCommitmentTape):
        ...
        def flatten_leaves(outputs):
            ...
            scalars = []
            for out in outputs:
                scalars.extend([out.O[0], out.O[1],
                                 out.I[0], out.I[1],
                                 out.C[0], out.C[1]])
        ...
        if isinstance(branches.root, RootBranchLeaves):
            ...
            root_vars = c1_tape.append_branch(len(branches.root.branch),
                                               list(branches.root.branch))
        else:
            root_vars = c2_tape.append_branch(len(branches.root.branch),
                                               list(branches.root.branch))
    ...
    @staticmethod
    def _transcript_inputs(branches: BranchesWithBlinds,
                           c1_tape: VectorCommitmentTape) -> list:
    ...
    @staticmethod
    def _transcript_blinds(branches: BranchesWithBlinds,
                           c1_tape: VectorCommitmentTape,
                           c2_tape: VectorCommitmentTape):
        ...
        for blind in branches.branches_2_blinds:
            ...
            dv, ddiv, dpt, dpad = c1_tape.append_claimed_point(
                c2p.scalar_bits, c2p.yx_coefficients, c2p.x_coefficients,
            ...
            c1_claims.append(PointWithDlog(
                dlog=dv, divisor=Divisor.from_tape_dict(ddiv), point=dpt
            ))

        # c2_tape opens C1 (Selene/HelioseleneField) branch blinds
        for blind in branches.branches_1_blinds:
            b = blind.inner    # PreparedBlind
            dv, ddiv, dpt, dpad = c2_tape.append_claimed_point(
                c1p.scalar_bits, c1p.yx_coefficients, c1p.x_coefficients,
                dlog    = b.scalar.decomposition,
                divisor = _poly_to_tape_dict(b.scalar_mul_and_divisor.divisor),
                point   = (b.scalar_mul_and_divisor.x, b.scalar_mul_and_divisor.y),
                padding = [],
            )
            c2_claims.append(PointWithDlog(
                dlog=dv, divisor=Divisor.from_tape_dict(ddiv), point=dpt
            ))
    ...
    @staticmethod
    def _input(params: FcmpParams, layers: int, transcript: ProverTranscript,
               c1_circuit, c1_dlog_challenge, c2_circuit, c2_dlog_challenge,
               root_vars: list,
               c1_branch_vars: list, c2_branch_vars: list,
               ...
               input_tuple: dict, transcripted_input: dict):
        ...
        # C1 (Selene): a = -3, b = SELENE_B in HelioseleneField
        c1_spec = CurveSpec(HelioseleneField(HelioseleneField.P - 3), SELENE_B)
        ...
        leaf_vars = (root_vars if layers == 1
                     else next(c1_branch_iter))
        leaf_chunks = [leaf_vars[i*6:(i+1)*6] for i in range(len(leaf_vars) // 6)]

        c1_circuit.first_layer(
            transcript, oc_spec,
            params.T_table, params.U_table, params.V_table, params.G_table,
            input_tuple["O_tilde"], ti["o_blind_claim"],   ti["O"],
            input_tuple["I_tilde"], ti["i_blind_u_claim"], ti["I"],
            input_tuple["R"],       ti["i_blind_v_claim"], ti["i_blind_blind_claim"],
            input_tuple["C_tilde"], ti["c_blind_claim"],   ti["C"],
            leaf_chunks,
        )
        ...
        for branch_vars in these_c2:
            ...
            if prior_c1_blind is not None:
                ...
                hash_witness = (hash_pt.x, hash_pt.y)  # HelioseleneField elements
            else:
                hash_witness = None
            ...
            c2_circuit.additional_layer(
                c1_spec,
                c2_dlog_challenge[0],
                (prior_c.x, prior_c.y),   # C1 point coords as HelioseleneField
                blind_opening,
                (hash_x, hash_y),
                branch_vars,
            )
    ...
    @classmethod
    def prove(cls, rng_seed: int, params: FcmpParams,
              branches: BranchesWithBlinds) -> "Fcmp":
        ...
        def _random_field_c2():
            return HelioseleneField(int.from_bytes(secrets.token_bytes(64), "little") % HelioseleneField.P)
        ...
        layers = ((1 if data0.leaves is not None else 0)
        ...
        c2_tape = VectorCommitmentTape(HelioseleneField, c2_padded)

        # Fill tapes
        per_input_vars, root_vars = cls._transcript_branches(
            branches, c1_tape, c2_tape)
        transcripted_inputs = cls._transcript_inputs(branches, c1_tape)
        c1_blind_claims, c2_blind_claims = cls._transcript_blinds(
            branches, c1_tape, c2_tape)

        # Build pvc_blinds — branch blinds are negated
        # ScalarDecomposition.scalar is already a Python int
        pvc_blinds_1 = [-b.inner.scalar.scalar % HeliosField.P
                        for b in branches.branches_1_blinds]
        pvc_blinds_2 = [-b.inner.scalar.scalar % HelioseleneField.P
                        for b in branches.branches_2_blinds]

        # Convert to field elements
        pvc_blinds_1 = [HeliosField(v) for v in pvc_blinds_1]
        pvc_blinds_2 = [HelioseleneField(v) for v in pvc_blinds_2]
        ...
        if is_c1:
            ...
            c = transcript.challenge(HelioseleneField)
            s = HelioseleneField((int(root_r) + int(c) * int(root_blind)) % HelioseleneField.P)
        ...
        c1_pvc_list = [
            ...
            for vals, b in zip(c1_tape.commitments, pvc_blinds_1)
        ]
        c2_pvc_list = [
            PedersenVectorCommitment(vals, b)
            for vals, b in zip(c2_tape.commitments, pvc_blinds_2)
        ]
        c1_circuit = Circuit.prove(HeliosField, c1_pvc_list)
        c2_circuit = Circuit.prove(HelioseleneField, c2_pvc_list)
        ...
        for idx, (data, ti) in enumerate(zip(branches.per_input, transcripted_inputs)):
            ...
            cls._input(
                params, input_layers, transcript,
        ...
        c2_stmt.prove(_random_field_c2, transcript, c2_witness, HelioseleneField)
    ...
    @classmethod
    def verify(cls, proof: bytes, root_blind_pok: bytes,
               params: FcmpParams,
               is_c1: bool, tree_root_bytes: bytes,
               layers: int,
               inputs: list,
               verifier_1, verifier_2,
               rng_fn) -> None:
        ...
        for _ in range(n_c1_blind_claims):
            dv, ddiv, dpt, _ = c1_tape.append_claimed_point(
                c2p.scalar_bits, c2p.yx_coefficients, c2p.x_coefficients,
                None, None, None, None)
            c1_blind_claims.append(PointWithDlog(
                dlog=dv, divisor=Divisor.from_tape_dict(ddiv), point=dpt))
        ...
        for _ in range(n_c2_blind_claims):
            dv, ddiv, dpt, _ = c2_tape.append_claimed_point(
                c1p.scalar_bits, c1p.yx_coefficients, c1p.x_coefficients,
                None, None, None, None)
            c2_blind_claims.append(PointWithDlog(
                dlog=dv, divisor=Divisor.from_tape_dict(ddiv), point=dpt))
        ...
        C1_pts, _ = transcript.read_commitments(
            len(c1_tape.commitments), 0, selene_from_bytes)
        C2_pts, _ = transcript.read_commitments(
            len(c2_tape.commitments), 0, helios_from_bytes)
        ...
        if is_c1:
            ...
            s    = HelioseleneField.from_bytes(root_blind_pok[32:64])
            c    = transcript.challenge(HelioseleneField)
            w    = HelioseleneField(int(rng_fn()))  # C2 (Helios) scalar = HelioseleneField
        ...
        c2_circuit = Circuit.verify(HelioseleneField)
        c1_dlog_ch = [None]
        c2_dlog_ch = [None]
        ...
        # C1 (Selene) scalar field = HeliosField; C2 (Helios) scalar field = HelioseleneField.
        def _rng_c1():
            return HeliosField(int(rng_fn()))

        def _rng_c2():
            return HelioseleneField(int(rng_fn()))

        c1_stmt, _ = c1_circuit.statement(c1_gens, C1_pts)
        c1_stmt.verify(
            _rng_c1, verifier_1, transcript,
            HeliosField, selene_from_bytes)

        c2_stmt, _ = c2_circuit.statement(c2_gens, C2_pts)
        c2_stmt.verify(
            _rng_c2, verifier_2, transcript,
            HelioseleneField, helios_from_bytes)
...
def _wpoint_to_xy_helioselene(pt: WPoint):
    x, y = pt.to_affine()
    return (HelioseleneField(x), HelioseleneField(y))
