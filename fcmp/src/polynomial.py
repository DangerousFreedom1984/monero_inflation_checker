...
class Poly:
    ...
    def _ensure_y(self, n):
        while len(self.y) < n:
            self.y.append(self._zero())

    def _ensure_yx(self, i, j):
        while len(self.yx) <= i:
            self.yx.append([])
        while len(self.yx[i]) < j:
            self.yx[i].append(self._zero())

    def _ensure_x(self, n):
        while len(self.x) < n:
            self.x.append(self._zero())
    ...
    def _shift_by_x(self, power):
        ...
        for _ in range(power):
            # Insert zero at the front of every x-coefficient list
            res.x.insert(0, z)
            new_yx = []
            for row in res.yx:
                new_row = list(row)
        ...
        # Ensure yx has enough slots
        while len(res.yx) < len(res.y):
            row = [z] * power
            res.yx.append(row)
        # Ensure each yx row has ≥ power elements
    ...
    # ------------------------------------------------------------------
    # dump (hex key=value lines, matching poly.rs output)
    # ------------------------------------------------------------------

    def dump(self, name):
        lines = []
        lines.append(f"{name}.zero_coefficient={self.zero.to_bytes().hex()}")
        y_hex = ",".join(c.to_bytes().hex() for c in self.y)
        lines.append(f"{name}.y_coefficients=[{y_hex}]")
        for i, row in enumerate(self.yx):
            row_hex = ",".join(c.to_bytes().hex() for c in row)
            lines.append(f"{name}.yx_coefficients[{i}]=[{row_hex}]")
        x_hex = ",".join(c.to_bytes().hex() for c in self.x)
        lines.append(f"{name}.x_coefficients=[{x_hex}]")
        return "\n".join(lines)
