...
proof1.json is a raw Monero transaction (RPC format)
"""
...
def extract(path: str):
    ...
    for idx, inp in enumerate(tx["vin"]):
        ...
        print(
            f"    key_offsets : {key['key_offsets']}  (ring member indices; empty = FCMP++ single-input)"
        )
        print(f"    k_image     : {key['k_image']}")
        print("                  (32-byte key image; unique per spent output)")
    ...
    for idx, out in enumerate(tx["vout"]):
        ...
        if "carrot_v1" in target:
            cv = target["carrot_v1"]
            print("    type     : carrot_v1  (Monero Seraphis-era output format)")
            print(f"    key      : {cv['key']}  (one-time public key)")
            print(f"    view_tag : {cv['view_tag']}  (3-byte scanning shortcut)")
            print(
                f"    enc_janus: {cv['encrypted_janus_anchor']}  (16-byte janus anchor, encrypted)"
            )
    ...
    print("  (may contain tx public key, additional keys, payment ID, nonce)")
    ...
    print("  (block height whose UTXO tree root anchors the FCMP++ proof)")
    ...
    print("  (even layers → root on C2/Helios; odd → root on C1/Selene)")
    ...
    print(
        f"  expected size     : {expected}  (proof_size(inputs={n_inputs}, layers={fcmp_layers}))"
    )
    ...
    print("    (Schnorr PoK proving knowledge of the tree root blinding factor)")
    print("    R = r·H  (nonce commitment on C1 or C2 generator H)")
    print("    s = r + c·blind  (response, c = Fiat-Shamir challenge)")
    ...
    print("    • Per-input (O_tilde, I_tilde, R, C_tilde) tuples (from wallet+chain)")
    print("    • FcmpParams (public parameters; consensus-fixed for mainnet)")
    print()


if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit(f"usage: {sys.argv[0]} <proof.json>")
    extract(sys.argv[1])
