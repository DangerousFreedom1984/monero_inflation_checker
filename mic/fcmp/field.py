...
HeliosField = Ed25519 base field  (p = 2^255 - 19)
SeleneField = the Selene base field (p = 0x7FFF...DF53)

Both use little-endian 32-byte encoding
"""
...
class HeliosField:
    ...
    def sqrt(self):
        ...
        Does NOT normalize to even. The caller adjusts for sign.
        """
        p = self.P
        exp = (p + 3) // 8  # = 2^252 - 2
    ...
    def from_uniform_bytes(cls, b: bytes) -> "HeliosField":
        """64-byte LE integer reduced mod p. """
        if len(b) != 64:
