...
import copy
from mic.common.df25519 import Point, PointVector
import numpy as np
import os
...
# .npy generators live next to this module (common/) 
_HERE = os.path.dirname(os.path.abspath(__file__))


def _npy(name):
    rows = np.load(os.path.join(_HERE, name), allow_pickle=False)
    return [Point(bytes(row)) for row in rows]
...
def setup_logger(name, log_file, level=logging.INFO):
    ...
    handler = logging.FileHandler(paths.log_path(log_file))
...
#
# Kept for reference: this is how the .npy files above were produced. Running it
# again needs hash_to_point and cn_fast_hash from df25519, df25519 itself, and
# varint_mic, none of which this module imports any more.
# --------------------------------------------------------------------------------------------
# M,N = 16,64
