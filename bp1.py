...
def verify(proofs,N):
    ...
    # H = hash_to_point(cn_fast_hash(strH.encode('utf-8').hex()))
    H = Scalar(8) * Point(cn_fast_hash(str(G)))
    Gi = PointVector([hash_to_point(cn_fast_hash(('pybulletGi' + str(i)).encode('utf-8').hex())) for i in range(max_MN)])
    Hi = PointVector([hash_to_point(cn_fast_hash(('pybulletHi' + str(i)).encode('utf-8').hex())) for i in range(max_MN)])
