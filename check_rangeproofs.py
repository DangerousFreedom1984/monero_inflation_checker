...
def check_sig_bp1(resp_json):
    ...
    if not verified:
        print('Potential inflation in Bulletproofs Signatures! Please verify what is happening!')
        with open("error.txt", "a+") as file1:
            # Writing data to a file
            file1.write(str(resp_json))
...
def check_bp_plus(proofs):
    ...
    for proof in proofs:
        ...
        H_scalar += weight*(r1*y*s1 + e**2*(y**(M*N+1)*z*one_MN**d + (z**2-z)*sum_y))
        G_scalar += weight*d1
        ...
        for i in range(MN):
            ...
            h_scalar = e*s1*weight*challenges_cache[(~i) & (MN-1)] - (e**2 * z * weight)
...
def prove_bp_plus(sv,gamma):
    ...
    domain = str("bulletproof_plus")
    H = Scalar(8) * Point(cn_fast_hash(str(G)))
    Hi_plus = PointVector([hash_to_point(cn_fast_hash(str(H) + domain.encode("utf-8").hex() + varint.encode_as_varint(i))) for i in range(0,2*M*N,2)])
    Gi_plus = PointVector([hash_to_point(cn_fast_hash(str(H) + domain.encode("utf-8").hex() + varint.encode_as_varint(i))) for i in range(1,2*M*N+1,2)])
    ...
    ip_data = InnerProductRound(Gi_plus,Hi_plus,G,H,aL1,aR1,alpha1,y,transcript,seed)
    while True:
        inner_product(ip_data)

        # We have reached the end of the recursion
        if ip_data.done:
            return BulletproofPlus(V,A,ip_data.A,ip_data.B,ip_data.r1,ip_data.s1,ip_data.d1,ip_data.L,ip_data.R,seed,gamma)
