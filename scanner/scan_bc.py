...
def start_scanning(filename, h, scan):
    ...
    filename_save_stats = mic_paths.stats_path("block_stats.csv")
    ...
    while True:
        ...
        settings_df25519.logger_basic.info(
            "Block: %s Txs: %s Duration(s): %s", h, nbr_txs, time_verify
        )
...
def start_scanning_precheck(filename, h, scan):
    ...
    filename_save_stats = mic_paths.stats_path("block_stats_pc.csv")
...
def txs_to_benchmark():
    ...
    txs.append(
        "9eeb8f29c7d1ca09462c135958f986fd47e6fc2f5bc67f7878bb55ef394cf36e"
    )  # with a point not on curve
    ...
    settings_df25519.logger_basic.info("Total execution time: %s", time.time() - t1)
...
def get_coinbase_sum(initial_block_height, count):
    ...
    settings_df25519.logger_basic.info("coinbase sum: %s", resp_sum)
    return resp_sum
